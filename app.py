...
import json, requests, os, tempfile, hashlib
...
# Function to pin an image to IPFS using Pinata
...
def create_nft():
    ...
    if res['IpfsHash']:
        ...
        if jsres['IpfsHash']:
            ...
            nft_mint = transaction.AssetCreateTxn(
                PUBLIC_ADDRESS, algod_client.suggested_params(), 1, 0, False,
                unit_name=unit_name, asset_name=asset_name,
                url="ipfs://"+jsres['IpfsHash']+"#arc3", metadata_hash=bytes.fromhex(digest)
            )
...
@app.route('/create_arc3', methods=['GET'])
def create_arc3():
    img_name = 'new.png'
    res = pin_image(img_name)
    image_integrity = sha256_hash_file("./docs/" + img_name)
    ...
    if res['IpfsHash']:
        ...
        if jsres['IpfsHash']:
            ...
            nft_mint = transaction.AssetCreateTxn(
                PUBLIC_ADDRESS, algod_client.suggested_params(), 1, 0, False,
                unit_name="MOONS", asset_name="ALGO-MOON",
                url="ipfs://"+jsres['IpfsHash']+"#arc3", metadata_hash=bytes.fromhex(digest)
            )
            signed_nft_mint = nft_mint.sign(mnemonic.to_private_key(MNEMONIC))
            tx_id = algod_client.send_transaction(signed_nft_mint)
            results = transaction.wait_for_confirmation(algod_client, tx_id, 4)
            created_asset = results["asset-index"]
            return json.dumps({"status": True, "notify": "Created Asset With id : {}".format(str(created_asset))})
        else:
